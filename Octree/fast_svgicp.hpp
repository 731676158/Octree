#ifndef FAST_GICP_FAST_SVGICP_HPP
#define FAST_GICP_FAST_SVGICP_HPP
...
/**
 * @brief Fast Source Voxelized GICP algorithm boosted with OpenMP
 */
template<typename PointSource, typename PointTarget>
class FastSVGICP : public FastGICP<PointSource, PointTarget> {
public:
  using Scalar = float;
  ...
  using Ptr = pcl::shared_ptr<FastSVGICP<PointSource, PointTarget>>;
  using ConstPtr = pcl::shared_ptr<const FastSVGICP<PointSource, PointTarget>>;
#else
  using Ptr = boost::shared_ptr<FastSVGICP<PointSource, PointTarget>>;
  using ConstPtr = boost::shared_ptr<const FastSVGICP<PointSource, PointTarget>>;
#endif
...
public:
  ...
  virtual void setInputSource(const PointCloudTargetConstPtr& cloud) override;
...
protected:
  ...
  std::vector<std::pair<Eigen::Vector4d, Eigen::Matrix4d>> source_mean_cov_;
